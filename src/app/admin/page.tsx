...
function SummaryCard({
    ...
    return (
        <Link
            href={link}
            className="rounded-xl bg-black bg-opacity-40 border border-gray-800 p-6 transition-all hover:border-gray-400/30 hover:shadow-lg hover:shadow-gray-400/5 backdrop-blur-sm"
        >
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center">
                    <div className={`p-3 rounded-lg bg-gradient-to-r ${gradientFrom} ${gradientTo} bg-opacity-20 text-gray-900`}>
                        {icon}
                    </div>
                    <h3 className="ml-3 text-sm font-semibold uppercase tracking-wider text-gray-400">{title}</h3>
                </div>
                {trend && (
                    <div className={`text-sm ${trend.positive ? 'text-emerald-400' : 'text-rose-400'} flex items-center`}>
                        <span className="mr-1">
                            {trend.positive ? (
            ...
            <div className="mb-1 text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-300 via-white to-gray-400">
                {value}
            </div>
    ...
    return (
        <div className="rounded-xl bg-black bg-opacity-40 border border-gray-800 backdrop-blur-sm overflow-hidden">
            <div className="p-6 border-b border-gray-800">
                <h2 className="text-lg font-medium text-transparent bg-clip-text bg-gradient-to-r from-gray-300 via-white to-gray-400">
                    {title}
                </h2>
            </div>
                ...
                <table className="w-full">
                    <thead className="bg-gray-900/50">
                        <tr className="text-left">
                            {columns.map((column) => (
                                <th key={column.key} className="px-6 py-3 text-xs font-medium uppercase tracking-wider text-gray-400">
                                    {column.label}
                                </th>
                            ))}
                        </tr>
        ...
        return (
            <div className="flex justify-center items-center h-64">
                <div className="text-center">
                    ...
                    <h3 className="text-xl font-medium text-gray-300 mb-2">Veriler Yüklenemedi</h3>
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
                <div>
                    <h1 className="text-3xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-gray-300 via-white to-gray-400">
                        Hoş Geldiniz!
                    </h1>
                    <p className="mt-1 text-gray-400">Kudat Steel Jewelry yönetim panelinde son durumu görebilirsiniz.</p>
                </div>
                <div className="mt-4 md:mt-0 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                    <div className="flex space-x-1 bg-gray-800 bg-opacity-50 rounded-lg p-1 mb-3 sm:mb-0">
                        {[
                            ...
                            <button
                                ...
                                className={`px-3 py-1 text-sm rounded-md ${period === option.value
                    ...
                    <div className="flex space-x-3">
                        <button className="px-4 py-2 rounded-lg bg-gray-500/10 text-gray-300 border border-gray-500/30 hover:bg-gray-500/20 transition-colors flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                            </svg>
                            Rapor İndir
                        </button>
                        <button className="px-4 py-2 rounded-lg bg-gradient-to-r from-gray-300 to-gray-400 text-gray-900 font-medium hover:from-gray-400 hover:to-gray-500 transition-colors flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    renderRow={(customer, index) => (
                        <tr key={customer._id} className={`${index % 2 === 0 ? 'bg-black bg-opacity-20' : ''} hover:bg-gray-800/40 transition-colors`}>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    <div className="h-8 w-8 rounded-full bg-gradient-to-r from-gray-700 to-gray-800 flex items-center justify-center text-gray-300 text-sm">
                ...
                <DataTable
                    ...
                    renderRow={(campaign, index) => (
                        <tr key={campaign._id} className={`${index % 2 === 0 ? 'bg-black bg-opacity-20' : ''} hover:bg-gray-800/40 transition-colors`}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                ...
                                >
                                    {campaign.status === "active" ? (
                                        <>
                                            <span className="w-1 h-1 mr-1.5 bg-emerald-400 rounded-full animate-pulse"></span>
                                            Aktif
                                        </>
                                    ) : campaign.status === "completed" ? (
                                        "Tamamlandı"
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <p className="text-sm text-gray-400">{campaign.sentEmails}</p>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                    <div className="w-16 bg-gray-800 rounded-full h-2.5 mr-2">
                                        <div
                                            className="bg-gradient-to-r from-gray-300 to-gray-400 h-2.5 rounded-full"
